# Fenrir's Chains: Reck Safety Boundaries
...
## Constraint Checker (Fenrir)

Fenrir validates every proposed action before execution. Constraints are defined in YAML and hot-reloadable without restart.
...
Fenrir rejects any action that violates a constraint and logs the rejection with the specific rule that triggered it.

## First-Time Fix Rule

Any fix that Reck has never applied before requires human approval. The system checks Valhalla (the decision archive) for prior instances of the same action on the same parameter class. If no precedent exists, Heimdall escalates via Bifrost.
...
2. **Apply**: Write the new setpoint via OPC-UA (Gungnir)
...
4. **First-time fix**: The proposed action has no precedent in Valhalla
5. **Multi-system disruption**: The proposed fix affects parameters on more than one production cell or line

Each escalation packages full context via Heimdall: signal history, causal graph excerpt, candidate actions, confidence scores, and risk assessment.

## Cascade Protection (Jormungandr)

The World Serpent watches for fixes that cause new anomalies.

### Circuit Breaker

Jormungandr tracks the causal chain of actions and their consequences:
...
If **three consecutive fixes each cause a new anomaly**, Jormungandr trips the circuit breaker:

- **Halt all autonomous action** on the affected line
- **Revert** the last applied fix
- **Escalate everything** to human review via Bifrost
...
Jormungandr identifies cascades by monitoring temporal correlation: if a new anomaly appears within the monitoring window of a recently applied fix, and the anomaly's causal subgraph overlaps with the fix's target parameters, Jormungandr flags it as a potential cascade link.
